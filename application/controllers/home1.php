...
*/
	...
	function wcategory_jobs($param1 = ''){
		if ($this->session->userdata('user_login') != 1)
        redirect(base_url() . 'home/login', 'refresh');
			$page_data['recent_jobs']   = $this->db->get_where('jobs', array('category_id' => $param1,'jwork_status' => 1))->result_array();
			$page_data['head']='dash_header.php';
			$page_data['foot']='dash_footer.php';
	...
	function pfavorite_jobworkers()
	{
		...
		$user_id =  $this->session->userdata('user_id');
		$page_data['recent_jobs'] = $this->db->get_where('jobs', array('jposter_id' => $user_id,'jposter_fav'=>1))->result_array();		
		$page_data['head']='dash_header.php';
        $page_data['foot']='dash_footer.php';
	...
	function ppast_jobworkers()
	{
		...
		$user_id =  $this->session->userdata('user_id');
		//$this->db->where("jcomplete_date !='0000-00-00'");
		$page_data['recent_jobs'] = $this->db->get_where('jobs', array('jposter_id' => $user_id))->result_array();		
		$page_data['head']='dash_header.php';
        $page_data['foot']='dash_footer.php';
	...
	function pprofile_view($param1 = '')
	{
		...
		$this->db->where('users.user_id',$param1); // Set Filter
		$page_data['user_profile'] = $this->db->get()->result_array();
		//$page_data['user_profile'] = $this->db->get_where('user_profiles', array('user_id' => 1))->result_array();		
		$page_data['head']='dash_header.php';
        $page_data['foot']='dash_footer.php';
